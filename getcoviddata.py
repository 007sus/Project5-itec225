...
def processJSON(jsdata):
    ...
    for i in data:
        ...
        if str(i['positive']).isdigit():
            vadeath.append(int(i['positive']))
        else:
            vadeath.append(0)
        if str(i['positiveCasesViral']).isdigit():
            vadeathconfirm.append(int(i['positiveCasesViral']))
        else:
...
def writeData( a, b, c, nf ):
    ...
    head = "Date,"+ nf.upper() + "DATA.positive," +  nf.upper() + "DATA.positiveCasesViral\n"
